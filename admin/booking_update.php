...
if(!empty($_REQUEST['insert'])){
    ...
    $old_lux_id=$_REQUEST['old_lux_id'];
    ...
    if($count == 0){
       header("location:nobooking.php");
        ...
        $room_no=$row['room_no'];
       $res2=getBooking($book_id);
       $row2=mysqli_fetch_assoc($res2);
       $old_room_id=$row2['room_id'];
       $q="update one_room set isoccupied='0' where id='$old_room_id' ";
       mysqli_query($conn,$q);

       $q="update one_room set c_in='$c_in',c_out='$c_out',isoccupied='1' where id='$room_no'";
       mysqli_query($conn,$q);
       $q="update  booking set room_id='$room_no',adults='$adults',children='$children',c_in='$c_in',c_out='$c_out' where id='$book_id'";
       mysqli_query($conn,$q);
       $q="update  cus_book set cus_id='$cus_id' where book_id='$book_id'";
       mysqli_query($conn,$q);
...
<div id="wrapper">
    ...
    <?php
    $old_cus_id=$_REQUEST['old_cus_id'];
    $book_id=$_REQUEST['book_id'];
    ?>
    ...
    <div id="page-wrapper">
        ...
        <div class="row">
            ...
            <form role="form" method="post" action="booking_update.php">
                <h4>change  customer update </h4>
                <div class="modal-body">
                    <div class="form-group">

                        <label>customer       :</label><?php echo $old_cus_id; ?></span><br>
                ...
                <?php $res=getBooking($book_id); ?>
                <?php $row=mysqli_fetch_assoc($res); ?>

                <div class="form-group">
                    <label>Check In Date</label>
                    <input class="form-control" value="<?php echo $row['c_in']; ?>" placeholder="YYYY-MM-DD"  pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" size="35"
                           name="c_in"   required>
                </div>
                <div class="form-group">
                    <label>Ckeck Out Date</label>
                    <input  class="form-control" value="<?php echo $row['c_out']; ?>" placeholder="YYYY-MM-DD"  pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" size="35"
                            name="c_out"   required>
                </div>
                <div class="form-group">
                    <label>Adults   :</label><span><?php echo $row['adults']; ?></span><br>
                ...
                <div class="form-group">
                    <label>Children    :</label><?php echo $row['children']; ?></span><br>
                ...
                <input type="hidden" name="old_lux_id" value="<?php echo $old_cus_id; ?>" >
